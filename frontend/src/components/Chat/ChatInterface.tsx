...
export function ChatInterface({ sessionId }: { sessionId: string | null }) {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Create "User Uploaded" message to START the chat
    const userMsg: Message = { 
        id: Date.now().toString(), 
        role: 'user', 
  ...
  // Logic to show/hide the Input Bar
      ...
      {!isChatActive && (
        <div className="absolute inset-0 flex flex-col items-center justify-center p-8 z-10 overflow-y-auto">
            ...
            <p className="text-gray-400 mb-10 text-center max-w-xl">
              Upload your data to unlock intelligent, privacy-first analysis powered by our <span className="text-cyan-400">6-Agent Swarm</span>.
            </p>
            ...
            <div className="w-full max-w-xl mb-12">
                <label className="flex flex-col items-center justify-center w-full h-64 border border-dashed border-white/10 rounded-3xl bg-[#0F1117] hover:bg-[#151720] transition-all cursor-pointer group">
                    <div className="flex flex-col items-center justify-center pt-5 pb-6">
                        ...
                        <span className="px-6 py-2 bg-blue-600 hover:bg-blue-500 text-white rounded-lg font-medium transition-colors shadow-lg shadow-blue-900/30">
                            Select File
                        </span>
                    </div>
      ...
      {isChatActive && (
        <>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6 scroll-smooth">
                ...
                {loading && (
                    <div className="ml-16 mt-4 flex items-center gap-2 text-cyan-400 text-xs animate-pulse">
            ...
            {/* INPUT BAR (Fixed at Bottom - ONLY VISIBLE WHEN CHAT ACTIVE) */}
            <div className="flex-shrink-0 p-6 bg-[#0B0C15] border-t border-white/5 z-20">
                ...
                <input
                    ...
                    className="w-full bg-[#1A1D26] text-white rounded-xl pl-6 pr-14 py-4 border border-white/10 focus:border-cyan-500/50 outline-none transition-all shadow-xl"
                />
                <button 
                    type="submit" 
                    disabled={!input.trim() || loading}
                    className="absolute right-2 top-2 p-2.5 bg-gradient-to-r from-cyan-600 to-blue-600 rounded-lg text-white hover:opacity-90 disabled:opacity-50 transition-all"
                >
                    <Send size={18} />
                </button>
                </form>
